...
void sjf_scheduler()
{
    ...
    while(job_queue->size > 0)
    {
        ...
        if (p->arrival_time < cpu_counter) //check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY; //set flag to ready
            dequeue(job_queue, p->pid); //remove from job queue
            enqueue_orderby_burst_time(ready_queue, p); //move process from job queue to ready queue
        }
        else
        {
            break; //no more process has arrived yet
    ...
    while (current != NULL) //loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_burst_time(ready_queue, p); //move process from waiting queue to ready queue
            }
        }
    }
...
void priority_scheduler()
{
    ...
    for (size_t i = 0; i < job_queue->size; i++)
    {
        ...
        if (p->arrival_time < cpu_counter) //check if process has arrived
        {
            p->flag = PROCESS_FLAG_READY; //set flag to ready
            dequeue(job_queue, p->pid); //remove from job queue
            enqueue_orderby_priority(ready_queue, p); //move process from job queue to ready queue
        }
        else
        {
            break; //no more process has arrived yet
    ...
    while (current != NULL) //loop waiting queue to find READY flag processes
    {
        ...
        if (p->flag == PROCESS_FLAG_READY)
        {
            ...
            {
                enqueue_orderby_priority(ready_queue, p); //move process from waiting queue to ready queue
            }
        }
    }
